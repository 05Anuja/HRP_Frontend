...
const SilgateSubmissions = () => {
  ...
  // Function that fetch the silgate records/leads
  const fetchSubmissions = async ({
    ...
    try {
      ...
      // Clear any selections that no longer exist on this page
  ...
  const downloadCsv = async () => {
    ...
    try {
      const response = await Axios.get("/silgate/export", {
        params: {
      ...
      link.setAttribute(
        "download",
        `silgate_export_${new Date().toISOString().slice(0, 10)}.csv`,
      );
      ...
      toast.error(
        err?.response?.data?.message ||
          err?.message ||
          "Failed to download Silgate CSV.",
      );
    } finally {
  ...
  //   try {
  //     await Axios.patch(`/silgate/assign`, {
  //       ids: Array.from(selectedRows),
  //       hrId: bulkAssignHr,
  //     });
  //     toast.success("Selected candidates assigned successfully.");
  ...
  const submitBulkAssign = async () => {
    ...
    try {
      const response = await Axios.patch("/silgate/assign", {
    ...
    const fetchHrs = async () => {
      ...
      try {
        ...
        if (res.data && Array.isArray(res.data.hrUsers)) {
          setHrs(
            res.data.hrUsers.filter((hr) => hr.projects?.includes("Silgate")),
          );
        }
      } catch (err) {
    ...
    <div className="space-y-6 fade-in-slide relative min-h-full">
      ...
      <div className="rounded-2xl border border-silgate-outline-variant/25 bg-white shadow-sm overflow-hidden relative">
        ...
        "
                      ...
                      <td className="whitespace-nowrap px-6 py-4 text-silgate-primary">
                        {row.language}
                      </td>
                      ...
                      <td className="whitespace-nowrap px-6 py-4">
                        <span
                          className={`inline-flex px-2 py-0.5 rounded-full text-[9px] font-bold uppercase border
                            ${
                              row.disposition === "Interested Lineup"
                                ? "bg-emerald-50 text-emerald-700 border-emerald-200"
                                : row.disposition === "Not Interested"
                                  ? "bg-rose-50 text-rose-700 border-rose-200"
                                  : row.disposition === "Call Back"
                      ...
                      <td className="whitespace-nowrap px-6 py-4 text-silgate-secondary font-medium">
                        ...
                        >
                          {row.resumeStatus === "Sent" ? "Sent" : "Not Sent"}
                        </span>
                      </td>
                      ...
                      <td className="whitespace-nowrap px-6 py-4 text-left">
                        <button
                          onClick={() =>
                            navigate(`/submissions/silgate/edit/${row._id}`, {
                              state: row,
                            })
                          }
...
export default SilgateSubmissions;
