...
const InterviewStatusList = () => {
  ...
  // States for interview status listing
  ...
  const [totalInterviewStatuses, setTotalInterviewStatuses] = useState(0);

  // Delete modal state
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [interviewStatusToDelete, setInterviewStatusToDelete] = useState(null);
  const [deleting, setDeleting] = useState(false);

  // Fetch interview statuses from backend
  const fetchInterviewStatuses = async (pageToFetch = page) => {
    setLoading(true);
    try {
      const response = await Axios.get("/interview-status", {
        params: {
          page: pageToFetch,
          limit: pageSize,
      ...
      if (resData) {
        setInterviewStatuses(resData.data || []);
        setPage(resData.currentPage || 1);
        setTotalPages(resData.totalPages || 1);
        setTotalInterviewStatuses(resData.totalInterviewStatuses || 0);
      }
    } catch (err) {
      toast.error(
  ...
  // Fetch interview statuses on parameters change
  useEffect(() => {
    fetchInterviewStatuses(page);
  }, [page, pageSize, projectFilter]);

  // Trigger delete confirmation
  const confirmDelete = (interviewStatus) => {
    setInterviewStatusToDelete(interviewStatus);
    setDeleteModalOpen(true);
  };

  // Execute deletion
  const handleDelete = async () => {
    if (!interviewStatusToDelete || deleting) return;
    setDeleting(true);
    try {
      await Axios.delete(`/interview-status/${interviewStatusToDelete._id}`);
      toast.success("Interview status deleted successfully.");
      setDeleteModalOpen(false);
      setInterviewStatusToDelete(null);
      // Fetch interview statuses again (handle edge case where we delete the last item on the page)
      const isLastItemOnPage = interviewStatuses.length === 1 && page > 1;
      const nextPage = isLastItemOnPage ? page - 1 : page;
      fetchInterviewStatuses(nextPage);
    } catch (err) {
      toast.error(
        err?.response?.data?.message ||
          err?.message ||
          "Failed to delete interview status.",
      );
    } finally {
      setDeleting(false);
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="text-left">
          ...
          <h1 className="text-xl font-bold tracking-tight text-silgate-primary mt-1 flex items-center gap-2">
            ...
            Interview Status Directory
          </h1>
          <p className="text-xs text-silgate-secondary mt-1">
            Configure candidate interview statuses and filter permissions across
        ...
        <div className="flex items-center gap-3 self-start sm:self-auto">
          <button
            onClick={() => fetchInterviewStatuses(page)}
          ...
          <button
            onClick={() => navigate("/interviewstatus/create")}
      ...
      {loading ? (
        <div className="p-16 flex flex-col items-center justify-center gap-3 bg-white rounded-2xl border border-silgate-outline-variant/20 shadow-sm">
          ...
          <span className="text-xs font-medium text-silgate-secondary">
            Syncing interview statuses...
          </span>
        </div>
      ) : totalInterviewStatuses === 0 ? (
        <div className="p-16 flex flex-col items-center justify-center gap-2 text-center bg-white rounded-2xl border border-silgate-outline-variant/20 shadow-sm">
          ...
          <p className="text-sm font-semibold text-silgate-primary mt-1">
            No Interview Statuses Found
          </p>
          <p className="text-xs text-silgate-secondary max-w-xs mx-auto">
            {projectFilter
              ? "Try adjusting your filter tags."
              : "Start by registering your first interview status by clicking the Add Interview Status button."}
          </p>
        </div>
      ) : (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 text-left">
                      <div className="flex flex-wrap gap-1.5">
                        {Array.isArray(status.project) ? (
                          status.project.map((proj) => {
                    ...
                    <td className="whitespace-nowrap px-5 py-3.5 text-silgate-secondary font-semibold text-left">
                      <div className="flex items-center gap-1.5">
                        ...
                        />
                        <span>{status.createdBy?.name || "System"}</span>
                      </div>
                    </td>
                    ...
                    <td className="whitespace-nowrap px-5 py-3.5 text-right font-medium">
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          onClick={() =>
                            navigate(`/interviewstatus/edit/${status._id}`, {
                              state: status,
                            })
                          }
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-6 py-4 border-t border-silgate-outline-variant/10 bg-white">
            <div className="flex items-center gap-4">
              <p className="text-xs text-silgate-secondary whitespace-nowrap font-semibold">
                Showing{" "}
                {totalInterviewStatuses === 0 ? 0 : (page - 1) * pageSize + 1}–
                {Math.min(page * pageSize, totalInterviewStatuses)} of{" "}
                {totalInterviewStatuses}
              </p>
              ...
              <div className="space-y-1 text-left">
                ...
                <p className="text-xs text-silgate-secondary">
                  ...
                  <strong className="text-silgate-primary">
                    "{interviewStatusToDelete?.interviewStatus}"
                  </strong>
