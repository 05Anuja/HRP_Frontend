...
const DispositionForm = ({ initialData, onClose, onSuccess }) => {
    ...
    const validate = () => {
        ...
        if (!formData.disposition.trim()) {
            errs.disposition = "Disposition name is required.";
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEdit && initialData?._id) {
                await Axios.patch(`/disposition/${initialData._id}`, payload);
                toast.success("Disposition updated successfully.");
            } else {
                await Axios.post("/disposition", payload);
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-4">
                    ...
                    >
                        <input
                            ...
                            onChange={(e) => {
                                setFormData((prev) => ({ ...prev, disposition: e.target.value }));
                                if (errors.disposition) setErrors((prev) => ({ ...prev, disposition: "" }));
                            }}
