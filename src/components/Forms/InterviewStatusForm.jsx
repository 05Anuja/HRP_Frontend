...
const InterviewStatusForm = ({ initialData, onClose, onSuccess }) => {
  ...
  const validate = () => {
    ...
    if (!formData.interviewStatus.trim()) {
      errs.interviewStatus = "Interview status name is required.";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        interviewStatus: formData.interviewStatus.trim(),
        project: formData.project,
      };

      if (isEdit && initialData?._id) {
        await Axios.patch(`/interview-status/${initialData._id}`, payload);
        toast.success("Interview status updated successfully.");
      } else {
        await Axios.post("/interview-status", payload);
        toast.success("Interview status created successfully.");
      }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-4">
          ...
          >
            <input
              ...
              onChange={(e) => {
                setFormData((prev) => ({ ...prev, interviewStatus: e.target.value }));
                if (errors.interviewStatus) setErrors((prev) => ({ ...prev, interviewStatus: "" }));
              }}
          ...
          <div className="space-y-1.5 text-left">
            <label className="block text-[11px] font-semibold text-silgate-secondary/80">
              Campaign Availability <span className="text-red-500 ml-0.5">*</span>
            </label>
